...
where salary < 14000
and salary >= 10000
order by first_name desc, salary desc, commission_pct desc;
...
문제8.
전체 부서를 출력하려고 합니다. 순서는 부서이름이 긴 순서대로 출력해 보시오.
...
from countries
where upper(country_name) in ('ARGENTINA','CHINA','GERMANY','UNITED STATES OF AMERICA','JAPAN')
...
select  first_name,
        salary,
        substr(phone_number,1,3)||'-'||substr(phone_number,5,3)||'-'||substr(phone_number,9),
        hire_date
from employees
