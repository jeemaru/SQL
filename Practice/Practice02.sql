...
from employees
group by job_id
order by min(salary) asc, round(avg(nvl(salary,0)),0) desc;
...
select  department_id,
        ...
        max(salary),
        min(salary),
        avg(nvl(salary,0))-min(salary) "평균임금 - 최저임금"
from employees
group by department_id
having avg(nvl(salary,0))-min(salary) < 2000
order by avg(nvl(salary,0))-min(salary) asc;
...
from employees
group by manager_id
having round(avg(nvl(salary,0)),0) >= 5000
order by avg(nvl(salary,0)) asc;
